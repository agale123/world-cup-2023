...
export class ScheduleComponent implements AfterViewInit, OnDestroy {
  ...
  constructor(
    ...
    const sortedMatches = this.matchService
      .getMatches()
      .sort((a, b) => a.id - b.id);
  ...
  addProjection() {
    ...
    this.projected = this.projected
      ...
      .filter(
        (p) =>
          p.position !== position || p.country.charAt(0) !== country.charAt(0)
      );
  ...
  removeProjection(projection: Projection) {
    this.projected = this.projected.filter(
      (p) =>
        p.country !== projection.country || p.position !== projection.position
    );
    this.updateMatches();
  }
  ...
  getMatchedProjections(country: string) {
    if (country in this.formatMatchMap) {
      ...
      const matchedProjections = this.projected.filter((p) => {
    ...
    this.matches = this.matchService
      ...
      .filter((match) => {
        ...
        return countries.some((country: string) => {
          if (
            this.getMatchedProjections(match.away).includes(country) ||
          ...
          if (
            formattedCountry == this.countryService.formatCountry(match.home) ||
            formattedCountry == this.countryService.formatCountry(match.away)
          ) {
            return true;
          }
          return false;
